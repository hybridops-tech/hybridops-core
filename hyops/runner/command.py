...
class RunnerContext:
    ...
    @property
    def remote_dispatch_ssh_key_path(self) -> str:
        if not self.ssh_private_key_file:
            return ""
        return f".ssh/{Path(self.ssh_private_key_file).name}"
...
def _ssh_common_argv(ctx: RunnerContext) -> list[str]:
    ...
    elif ctx.ssh_access_mode == "bastion-explicit":
...
def _bundle_runtime(paths, bundle_path: Path) -> None:
    with tarfile.open(bundle_path, "w:gz") as tf:
        ...
        sibling_shared = paths.root.parent / "shared"
        if sibling_shared != paths.root and sibling_shared.exists():
            for name in _SYNC_DIRS:
                src = sibling_shared / name
                if not src.exists():
                    continue
                tf.add(src, arcname=f"envs/shared/{name}")
...
def _cleanup_local_stage(paths, run_id: str) -> None:
    stage_root = paths.work_dir / "runner" / run_id
    shutil.rmtree(stage_root, ignore_errors=True)


def _rebase_remote_runtime_path(value: str, *, remote_runtime_root: str, local_runtime_root: Path) -> str:
    token = str(value or "").strip()
    remote_root = str(remote_runtime_root or "").rstrip("/")
    if not token or not remote_root or not token.startswith(remote_root + "/"):
        return token
    suffix = token[len(remote_root) + 1 :]
    return str((local_runtime_root / suffix).resolve())


def _repair_returned_module_state_paths(*, local_runtime_root: Path, remote_runtime_root: str) -> None:
    state_root = (local_runtime_root / "state" / "modules").resolve()
    ...
    for state_file in state_root.rglob('*.json'):
        ...
        changed = False
        for key in ('rerun_inputs_file', 'evidence_dir'):
            current = str(payload.get(key) or '').strip()
            repaired = _rebase_remote_runtime_path(
                current,
...
def _write_dispatch_env_file(stage_root: Path, env_values: dict[str, str]) -> Path:
    env_path = stage_root / "dispatch.env"
    lines = ["# generated by hyops runner dispatch"]
    for key, value in sorted(env_values.items()):
        if "\n" in value:
            marker = f"__HYOPS_ENV_{key}__"
            lines.append(f"export {key}=$(cat <<'{marker}'")
            lines.append(value)
            lines.append(marker)
            lines.append(")")
        else:
...
def _sync_runtime_to_runner(
    ...
    *,
    ...
    local_stage_root, local_bundle = _local_bundle_paths(paths, run_id)
    _bundle_runtime(paths, local_bundle)
    local_dispatch_env = _write_dispatch_env_file(local_stage_root, dispatch_env or {})

    job_root, runtime_root, remote_bundle = _remote_bundle_paths(run_id)
    remote_dispatch_env = f"{job_root}/dispatch.env"
    ...
    if ctx.ssh_private_key_file:
        remote_key = ctx.remote_dispatch_ssh_key_path
        r = run_capture_stream(
            _ssh_argv(
                ctx,
            ...
            evidence_dir=evidence_dir,
            label="runner_prepare_ssh_key",
            tee_path=evidence_dir / "runner.log",
            redact=True,
        )
        if r.rc != 0:
            raise RuntimeError("failed to prepare runner ssh key directory")

        r = run_capture_stream(
            _scp_argv(ctx, ctx.ssh_private_key_file, remote_key, to_remote=True),
            evidence_dir=evidence_dir,
            label="runner_upload_ssh_key",
            tee_path=evidence_dir / "runner.log",
            redact=True,
        )
        if r.rc != 0:
            raise RuntimeError("failed to upload runner ssh private key")
        ...
        r = run_capture_stream(
            ...
            evidence_dir=evidence_dir,
            label="runner_fix_ssh_key_perms",
            tee_path=evidence_dir / "runner.log",
            redact=True,
        )
        if r.rc != 0:
            raise RuntimeError("failed to secure runner ssh private key")
    ...
    r = run_capture_stream(
        _scp_argv(ctx, str(local_dispatch_env), remote_dispatch_env, to_remote=True),
        evidence_dir=evidence_dir,
        label="runner_upload_dispatch_env",
        tee_path=evidence_dir / "runner.log",
        redact=True,
    )
    if r.rc != 0:
        raise RuntimeError("failed to upload runner dispatch env file")

    r = run_capture_stream(
        _ssh_argv(
            ctx,
            f"chmod 600 {shlex.quote(remote_dispatch_env)}",
        ),
        evidence_dir=evidence_dir,
        label="runner_fix_dispatch_env_perms",
        tee_path=evidence_dir / "runner.log",
        redact=True,
    )
    if r.rc != 0:
        raise RuntimeError("failed to secure runner dispatch env file")

    return job_root, runtime_root, remote_dispatch_env
...
def _sync_runtime_back(ctx: RunnerContext, *, paths, run_id: str, job_root: str, evidence_dir: Path) -> None:
    ...
    if local_result.exists():
        ...
        _repair_returned_module_state_paths(
            local_runtime_root=paths.root,
            remote_runtime_root=runtime_root,
        )
...
def _remote_blueprint_command(ns, *, runtime_root: str, remote_blueprint_path: str, ctx: RunnerContext) -> list[str]:
    ...
    cmd.extend(
        [
            "blueprint",
            ns.runner_blueprint_cmd,
            "--root",
            remote_root,
            "--file",
            remote_blueprint_path,
        ]
    )
...
def _execute_runner_blueprint(ns) -> int:
    ...
    job_root = ""
    completed_ok = False
    _job_root_preview, _runtime_root_preview, _remote_bundle_preview = _remote_bundle_paths(run_id)
    try:
        dispatch_env_values = {"HYOPS_CORE_ROOT": ctx.remote_core_root}
        env_name = str(getattr(ns, "env", "") or "").strip()
        if env_name:
            dispatch_env_values["HYOPS_ENV"] = env_name
        bundled_shared_root = f"{_runtime_root_preview}/envs/shared"
        if (paths.root.parent / "shared").exists():
            dispatch_env_values["HYOPS_SDN_AUTHORITY_ROOT"] = bundled_shared_root
            dispatch_env_values["HYOPS_NETBOX_AUTHORITY_ROOT"] = bundled_shared_root
        dispatch_env_values.update(tfc_env_values)
        dispatch_env_values.update(gcp_env_values)
        dispatch_env_values.update({key: sync_env_values[key] for key in sync_env_keys})
        job_root, runtime_root, remote_dispatch_env = _sync_runtime_to_runner(
            ctx,
            paths=paths,
            run_id=run_id,
            evidence_dir=evidence_dir,
            dispatch_env=dispatch_env_values,
        )
        ...
        remote_shell = (
            f"set -a && . {shlex.quote(remote_dispatch_env)} && set +a && "
        ...
        print(f"runner={ctx.state_ref} status=cancelled run_id={run_id}")
        print(f"run record: {evidence_dir}")
        ...
        if completed_ok:
            _cleanup_local_stage(paths, run_id)
