"""hyops.validators.platform.network.decision_consumer

purpose: Validate inputs for platform/network/decision-consumer module.
...
def validate(inputs: dict[str, Any]) -> None:
    ...
    _require_non_empty_str(data.get("consumer_role_fqcn"), "inputs.consumer_role_fqcn")
    state = _require_non_empty_str(data.get("consumer_state"), "inputs.consumer_state").lower()
    if state not in {"present", "absent"}:
        raise ValueError("inputs.consumer_state must be 'present' or 'absent'")
    if state == "absent":
        return

    _require_non_empty_str(data.get("consumer_root"), "inputs.consumer_root")
    _require_non_empty_str(
        data.get("consumer_dispatch_requests_dir"),
        "inputs.consumer_dispatch_requests_dir",
    )
    mode = _require_non_empty_str(
        data.get("consumer_execution_mode") or "approval-only",
        "inputs.consumer_execution_mode",
    ).lower()
    if mode != "approval-only":
        raise ValueError("inputs.consumer_execution_mode must be 'approval-only' in v1")
    require_approval = data.get("consumer_require_approval")
    if not isinstance(require_approval, bool):
        raise ValueError("inputs.consumer_require_approval must be a boolean")
    _require_int_ge(data.get("consumer_poll_seconds"), "inputs.consumer_poll_seconds", 5)
    _require_non_empty_str(data.get("consumer_log_level"), "inputs.consumer_log_level")
