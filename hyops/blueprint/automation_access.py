...
def _ssh_config(
    *,
    ...
    lines = ["# Generated by HybridOps.Core for this blueprint environment."]
    if not direct:
        if not gateway:
            raise ValueError("gateway is required for proxied automation access")
        lines.extend(
            [
    ...
    for target in targets:
        ...
        if not direct:
            entry.append(f"  ProxyJump {gateway_alias}")
        entry.extend(
            [
                "  StrictHostKeyChecking accept-new",
...
def prepare_automation_session(
    *,
    ...
    if raw_trust_scope:
        ...
        trust_seed = f"{(gateway or {}).get('known_hosts_file', '')}:{socks_port}:{direct}"
    ...
    proxy = "" if direct else f"socks5h://127.0.0.1:{socks_port}"
    proxy_lines = ["unset HYOPS_SOCKS_PROXY ALL_PROXY all_proxy"]
    if proxy:
        proxy_lines = [
        ...
        yaml.safe_dump(
            {
                ...
                "access_mode": "direct" if direct else "proxy",
                "socks_proxy": proxy,
            },
            sort_keys=False,
        ),
