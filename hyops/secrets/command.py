...
FILE_BACKED_SECRET_KEYS = frozenset(
    {
        "EVENG_IOL_LICENSE",
        "GNS3_IOU_LICENSE",
    }
...
def run_ensure(ns) -> int:
    ...
    file_backed_keys = [key for key in keys if key in FILE_BACKED_SECRET_KEYS]
    if file_backed_keys:
        env_name = str(getattr(ns, "env", None) or paths.root.name)
        for key in file_backed_keys:
            print(f"ERR: {key} cannot be generated by secrets ensure.")
            print(
                "Run: "
                f"hyops secrets set --env {env_name} "
                f"--from-file {key}=/path/to/iourc"
            )
        return CONFIG_INVALID
