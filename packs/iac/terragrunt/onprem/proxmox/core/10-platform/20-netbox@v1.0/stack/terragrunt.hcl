...
locals {
  ...
  vm_name      = trimspace(try(tostring(local.inputs.vm_name), "")) != "" ? trimspace(tostring(local.inputs.vm_name)) : "netbox"
  vm_id        = try(tonumber(local.inputs.vm_id), 302)
  vm_ipv4_cidr = trimspace(try(tostring(local.inputs.vm_ipv4_cidr), "")) != "" ? trimspace(tostring(local.inputs.vm_ipv4_cidr)) : "10.12.0.11/24"
  vm_gateway   = trimspace(try(tostring(local.inputs.vm_gateway), "")) != "" ? trimspace(tostring(local.inputs.vm_gateway)) : "10.12.0.1"
  vm_mac       = trimspace(try(tostring(local.inputs.vm_mac), "")) != "" ? trimspace(tostring(local.inputs.vm_mac)) : "BC:24:11:00:00:31"
  ...
  tags    = length(local.tags_in) > 0 ? local.tags_in : ["platform", "onprem", "netbox", "ipam", "critical"]
