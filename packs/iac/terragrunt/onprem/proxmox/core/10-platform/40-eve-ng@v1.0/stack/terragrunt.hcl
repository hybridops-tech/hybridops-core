...
locals {
  ...
  vm_name      = trimspace(try(tostring(local.inputs.vm_name), "")) != "" ? trimspace(tostring(local.inputs.vm_name)) : "eve-ng"
  vm_id        = try(tonumber(local.inputs.vm_id), 402)
  vm_ipv4_cidr = trimspace(try(tostring(local.inputs.vm_ipv4_cidr), "")) != "" ? trimspace(tostring(local.inputs.vm_ipv4_cidr)) : "10.10.0.41/24"
  vm_gateway   = trimspace(try(tostring(local.inputs.vm_gateway), "")) != "" ? trimspace(tostring(local.inputs.vm_gateway)) : "10.10.0.1"
  vm_mac       = trimspace(try(tostring(local.inputs.vm_mac), "")) != "" ? trimspace(tostring(local.inputs.vm_mac)) : "BC:24:11:00:00:41"

  mgmt_bridge  = try(local.inputs.network_bridge, "vmbr0")
  stage_bridge = trimspace(try(tostring(local.inputs.stage_network_bridge), "")) != "" ? trimspace(tostring(local.inputs.stage_network_bridge)) : "vmbr30"
  prod_bridge  = trimspace(try(tostring(local.inputs.prod_network_bridge), "")) != "" ? trimspace(tostring(local.inputs.prod_network_bridge)) : "vmbr40"

  default_interfaces = [
    {
      bridge      = local.mgmt_bridge
    ...
    {
      bridge = local.stage_bridge
      ipv4 = {
        address = "dhcp"
      }
    },
    {
      bridge = local.prod_bridge
      ipv4 = {
        address = "dhcp"
      }
    },
  ...
  tags    = length(local.tags_in) > 0 ? local.tags_in : ["platform", "onprem", "eve-ng", "network-emulation", "critical"]
...
inputs = {
  ...
  vm_id                = local.vm_id
  template_vm_id       = try(tonumber(local.inputs.template_vm_id), 9000)
  cpu_cores            = try(tonumber(local.inputs.cpu_cores), 6)
  cpu_type             = try(local.inputs.cpu_type, "host")
  memory_mb            = try(tonumber(local.inputs.memory_mb), 12288)
  disk_size_gb         = try(tonumber(local.inputs.disk_size_gb), 150)
